...
def load_patch(patch_path, return_roi=False, parent_dir=None):
    '''
    加载patches
    :param patch_path: 存储patches的路径
    :param return_roi: 判断是否需要范围patches对应的ROI，因为这里我们存储patches的名字和ROI的名称是对应的
    :param parent_dir: 是存放ROI的路径
    :return:
    '''
    if not return_roi:
...
def resize_images(images, size, rescale=True):
    ...
    :param images: 输入的图像
    :param size: 输出图像的大小
    :param rescale: 是否rescale，一般是将像素值的范围映射到[-1,1]
    :return:
    '''
    res = np.zeros(
        [
